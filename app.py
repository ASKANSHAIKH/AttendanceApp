...
if st.session_state.auth:
    ...
    if st.sidebar.button("⬅️ Change Role / Back"): st.session_state.nav = 'Role Select'; st.rerun()
...
# --------------------------------------------------------------------------------------------------
    ...
    if st.button("🧹 Clear Application Cache & Reconnect Database"):
        # This function clears the database cache, forcing a new connection session
...
# --------------------------------------------------------------------------------------------------
    ...
    # If session auth is lost or nav state is bad, redirect to role select
...
def calculate_salary_logic(emp_id, pay_month, pay_year, base_salary):
    ...
    while curr <= e_date:
        ...
        days += cred
        # NOTE: Date is kept as a datetime object for clean export
        report.append([curr, curr.strftime("%A"), stat, cred])
        curr += timedelta(days=1)
...
# --------------------------------------------------------------------------------------------------
# 7. ADMIN DASHBOARD - MAINTENANCE (New Tab)
# --------------------------------------------------------------------------------------------------
elif st.session_state.nav == 'Admin - Maint' and st.session_state.auth:
    st.title("Admin Dashboard: System Maintenance")
    st.warning("Use this section only if the app is slow or crashing frequently.")
